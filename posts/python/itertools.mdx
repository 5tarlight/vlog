...
한줄로 끝낼 수 있습니다. 매력적이지 않나요? 특히 시간이 중요한 PS에서는 많은 도움이 될 것
...
첫번째 인자로 data를 받고 두번째 인자로 함수 key를 받습니다. data의 요소를 하나씩 순회하며
...
첫번째로 iterable을 주고 두번째 인자로 정수 n을 주면 크기가 $n$인 배치를 만듭니다.
...
첫번째 인자로 람다나 함수 predicate를 받고 두번째 인자로 data를 받습니다.
...
iterator를 입력으로 받아서 두번째 인자인 n개의 iterator로 반환합니다. 각 iterator는

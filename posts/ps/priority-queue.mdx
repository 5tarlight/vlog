...
우선순위 큐는 힙(heap)이라는 자료구조를 통해 구현됩니다. 힙은 완전 이진 트리(Complete Binary
Tree)의 일종입니다. 트리를 설명하지 않아서 힙에 대해서 지금 자세히 설명하진 않겠지만,
2개의 자식으로 갈라지는 트리 형태를 가지고 있습니다. 힙에서는 데이터의 삽입, 삭제가
$O(\log n)$의 시간 복잡도로 가능합니다.

C++에서는 `priority_queue`가 지원되고 파이썬에서는 `heapq` 모듈이나
`queue.PriorityQueue` 클래스를 통해 사용할 수 있습니다. 하지만, `queue.PriorityQueue`는
BOJ의 python3 채점 환경에서는 PriorityQueue를 import조차 할 수 없으므로

...
각각의 간선이 주어집니다. 간선은 두 개의 정수 $u, v$로 주어지며, $u$와 $v$는 서로 연결되어 있음을
의미합니다. 노드는 0부터 $N - 1$까지의 정수로 표현됩니다. 그래프는 무방향 그래프입니다.
...
BFS를 0번 노드부터 시작해서 탐색한 순서대로 노드를 출력합니다.
...
격자는 굉장히 자주 등장하는 그래프의 특별한 형태입니다. 2차원 각 칸이 노드가 되고, 별다른 말이 없으면
상하좌우에 있는 노드(칸)끼리 연결되어 있는 그래프입니다.
...
인접한 노드의 위치를 dx, dy를 사용해서 구할 수 있습니다. nx, ny가 격자 범위를 벗어나지 않는지도
확인해야 합니다.
...
BFS로도 특정 경우에 최단 경로를 구할 수 있습니다. 앞으로 최단 경로 알고리즘도 다룰 예정이어서
여기서 설명하지는 않았습니다. 연습 문제에는 BFS를 통해서 최단 경로를 구하는 문제도 넣어 놨으니
도전해 보시기 바랍니다.
